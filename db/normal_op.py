...
def create_medical_record(medical_record_info):
    create_patient(medical_record_info.patient_info)
    create_contact(medical_record_info.contact_info)
    ...
    for ward_info in medical_record_info.ward_infos:
        create_recordAndWard(medical_record_id, ward_info)
    for cost_info in medical_record_info.cost_infos:
        create_cost(medical_record_id, cost_info)

def create_patient(patient_info):
    conn, cursor = make_connect()
    if ~if_patient_exist(patient_info.id_card_number): #避免重复新建
        insert_query = """
...
def create_recordAndWard(medical_record_id, ward_info):
    conn, cursor = make_connect()
    insert_query = """
            INSERT INTO MedicalRecordWards (MedicalRecordID, WardID, StartTime, EndTime)
            VALUES (%s, %s, %s, %s)
            """
    cursor.execute(insert_query, (medical_record_id, get_wardId_by_name(ward_info.ward_name), ward_info.start_time, ward_info.end_time))
    conn.commit()
    break_connect(conn, cursor)

def create_cost(medical_record_id, cost_info):
    conn, cursor = make_connect()
    insert_query = """
...
def get_record_by_recordID(recordID):
    ...
    medical_record_info = MedicalRecordInfo(
        ...
        unit_name=get_unitName_by_id(record[4]),
...
def get_contact_info(patientId):
    conn, cursor = make_connect()
...
def get_surgery_infos(medicalRecordId):
    ...
    for data in datas:
        surgery_info = SurgeryInfo(
            surgery_date=data[2],
            surgery_name=data[3],
            surgeon_name=get_staffName_by_id(data[4]),
            assistant_surgeon_name=get_staffName_by_id(data[5])
        )
        surgery_infos.append(surgery_info)
    return surgery_infos

def get_ward_infos(medicalRecordId):
    conn, cursor = make_connect()
    ...
    for data in datas:
        ward_info = WardInfo(
            ward_name=get_wardName_by_id(data[1]),
            start_time=data[2],
            end_time=data[3]
        )
...
def get_cost_infos(medicalRecordId):
    conn, cursor = make_connect()
