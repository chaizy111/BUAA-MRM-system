...
class Ui_MainWindow(object):
    ...
    def openQueryDialog(self):
        ...
        search_info = SearchInfo(
            ...
            patient_name=self.lineEdit_2.text(),  # 患者姓名
            gender=self.comboBox.currentText(),  # 性别
            payment_method=self.comboBox_2.currentText(),  # 医疗付款方式
            nationality=self.lineEdit_6.text(),  # 国籍
            ethnicity=self.lineEdit_7.text(),  # 民族
            occupation=self.lineEdit_8.text(),  # 职业
            address=self.lineEdit_9.text(),  # 地址
            phone=self.lineEdit_10.text(),  # 联系电话
            contact_name=self.lineEdit_12.text(),  # 联系人
            contact_address=self.lineEdit_13.text(),  # 联系人地址
            contact_phone= self.lineEdit_14.text(),  # 联系人电话
            birth_from= self.dateEdit.date().toString("yyyy-MM-dd"),  # 出生年月
            age=self.ui.spinBox.value(),  # 患者年龄
            admission_date=self.dateEdit_2.date().toString("yyyy-MM-dd"),  # 入院时间
            discharge_date=self.dateEdit_3.date().toString("yyyy-MM-dd"),  # 出院时间
            disease_name=self.lineEdit_15.text(),  # 疾病名称
            hospital_stay_days=self.spinBox_2.value()  # 住院天数
        )
